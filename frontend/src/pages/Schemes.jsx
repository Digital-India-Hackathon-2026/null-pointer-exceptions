...
import { Search, Bookmark, BookmarkCheck, ArrowRight, X } from 'lucide-react';
...
const Schemes = () => {
  ...
  const filtered = useMemo(() => {
    return SCHEMES.filter((s) => {
      ...
      const isPrivate = s.ministry?.startsWith('Private:');
      if (type === 'govt' && isPrivate) return false;
      if (type === 'private' && !isPrivate) return false;
      
      return true;
    });
  }, [q, cat, state, ministry, type]);
  ...
  const hasFilter = q || cat !== 'all' || state !== 'all' || ministry !== 'all' || type !== 'all';

  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          {/* Brighter Green Explore Label */}
          <div className="text-xs uppercase tracking-[0.16em] text-green-600 font-bold">Explore</div>
      ...
      <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 lg:flex">
            <Select key={`type-${type}`} value={type} onValueChange={setType}>
              <SelectTrigger className="h-11 lg:w-40">
                <SelectValue placeholder="Scheme Sector" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All Sectors</SelectItem>
                <SelectItem value="govt">Government</SelectItem>
                <SelectItem value="private">Private / CSR</SelectItem>
              </SelectContent>
            </Select>

            <Select key={`cat-${cat}`} value={cat} onValueChange={setCat}>
              <SelectTrigger className="h-11 lg:w-40">
            ...
            <Select key={`state-${state}`} value={state} onValueChange={setState}>
              <SelectTrigger className="h-11 lg:w-40">
            ...
            <Select key={`min-${ministry}`} value={ministry} onValueChange={setMinistry}>
              ...
              <SelectContent className="max-h-72">
                ...
                {MINISTRIES.map((m) => (
                  <SelectItem key={m} value={m}>
                    {m.replace('Ministry of ', '').replace('Private: ', '')}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        {/* Dynamic Badges with Brighter Green Styling */}
        {hasFilter && (
          <div className="mt-3 flex items-center justify-between">
            <div className="flex flex-wrap gap-2">
              {type !== 'all' && (
                <Badge variant="outline" className="gap-1 border-green-200 bg-green-50 text-green-700 font-medium">
                  Sector: {type === 'govt' ? 'Government' : 'Private'}
                </Badge>
              )}
              {cat !== 'all' && (
                <Badge variant="outline" className="gap-1 border-green-200 bg-green-50 text-green-700 font-medium">
                  Category: {CATEGORIES.find(c => c.id === cat)?.label || cat}
                </Badge>
              )}
              {state !== 'all' && (
                <Badge variant="outline" className="gap-1 border-green-200 bg-green-50 text-green-700 font-medium">
              ...
              {ministry !== 'all' && (
                <Badge variant="outline" className="gap-1 border-green-200 bg-green-50 text-green-700 font-medium">
                  {ministry.replace('Ministry of ', '').replace('Private: ', '')}
                </Badge>
              )}
            </div>
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filtered.map((s) => {
          ...
          return (
            <Card key={s.id} className="p-6 border border-slate-200 rounded-2xl bg-white flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between gap-3">
                  <Badge variant="outline" className="border-slate-200 bg-slate-50 text-slate-700 font-medium truncate max-w-[160px]">
                    {(s.ministry || '').replace('Ministry of ', '').replace('Private: ', '')}
                  </Badge>
                  
                  {/* FIXED: Conditional rendering ensures explicit sector display */}
                  ...
                  <button
                    ...
                    className="text-slate-400 hover:text-green-600 transition-colors ml-1"
                    aria-label="save"
                  >
              ...
              {/* View Action Link using High-contrast Green */}
              <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
                ...
                <Link to={`/schemes/${s.id}`} className="text-sm font-bold text-green-600 hover:text-green-700 inline-flex items-center gap-1">
